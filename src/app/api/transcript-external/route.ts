...
export async function POST(request: NextRequest) {
  try {
    ...
    // Get transcript from external API service
    ...
    try {
      ...
      const result = await transcriptService.fetchTranscript(finalVideoId, service as 'rapidapi' | 'assemblyai' | 'custom');
      
      transcriptItems = result.transcript;
      videoData = result.metadata;
      ...
      console.error('External transcript extraction failed:', transcriptError.message);
    }

    // If transcript extraction failed, return appropriate error
    if (transcriptError) {
      const errorMessage = transcriptError.message.includes('API request failed') 
        ? 'Failed to connect to transcript service'
        : transcriptError.message.includes('environment variable is required')
        ? 'Transcript service not configured properly'
        : transcriptError.message.includes('No transcript content available')
    ...
    // If we don't have video metadata from the external service, try to get it from YouTube Data API
    if (!videoData || !videoData.title || videoData.title === `YouTube Video - ${finalVideoId}`) {
      try {
        ...
        // Keep the metadata from external service if available
        if (!videoData) {
          videoData = {
    ...
    console.error('External transcript extraction error:', error);
    
    return NextResponse.json(
      { 
        error: 'Failed to extract transcript using external service. Please check your API configuration.',
